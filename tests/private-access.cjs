...
const context={getStore:()=>store,createHmac,timingSafeEqual,Buffer,Response,URL,Netlify:{env:{get:key=>key==='IFC_SESSION_SECRET'?secret:''}},console};vm.createContext(context);
...
(async()=>{
 assert.equal((await call(project)).status,200);
 assert.equal((await call(project)).status,200);
 assert.equal((await call(pdf)).status,200);
 ...
 assert.equal((await call(other,`?project=${project}`)).status,200);
 assert.equal((await call(project,'',{action:'finalize'})).status,403);
 assert.equal((await call(project)).status,200);assert.equal((await call(pdf,`?project=${project}&part=0`)).status,200);
 assert.equal((await call(project,'',undefined,cookie)).status,200);
 console.log('PASS: anonymous shared reads; owner-only writes; linked PDFs readable; no public cache.');
