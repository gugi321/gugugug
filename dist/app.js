...
const urlParams=new URLSearchParams(location.search),sharedId=urlParams.get('share'),embedded=$('embeddedIFC'),adminPath=location.pathname.replace(/\/+$/,'')==='/admin',publicLanding=!sharedId&&!embedded&&!adminPath;
// O visualizador publicado é somente leitura. Alterações e novos arquivos são feitos pelo proprietário no Netlify.
const readOnlyMode=true;
let ownerUser=null,ownerAuthorized=false,authReady=readOnlyMode,trustedReadOnlyLoad=false;
...
function showOwnerGate(message=''){if($('brandIntro'))$('brandIntro').hidden=true;if($('ownerGate'))$('ownerGate').hidden=false;if(message)$('ownerStatus').textContent=message;}
function publicUI(){document.body.classList.toggle('readonly',readOnlyMode);if(readOnlyMode){document.querySelectorAll('#share,#file,.open,#ownerLogout,#uploadPDFs,#pdfFiles,#exportIFC,#exportHTML,#exportPNG,#shareZIP,#shareHTML,#saveHTML,#saveZIP,#pdfDownload,#printElement,[data-owner-control]').forEach(el=>{el.hidden=true;el.disabled=true});$('brandEnter').textContent=publicLanding?'Área administrativa':'Abrir visualização';$('brandIntroHint').hidden=true;}}
...
function ownerReady(user){ownerUser=user;ownerAuthorized=true;authReady=true;document.body.classList.add('owner-ready');document.body.classList.remove('locked');$('ownerGate').hidden=true;hideBrandIntro();$('ownerAccount').hidden=false;$('ownerAccount').textContent=user?.email||'Proprietário';$('ownerLogout').hidden=false;$('brandEnter').hidden=true;status('');}
function ownerLocked(message='Entre com a conta autorizada para carregar arquivos.'){ownerUser=null;ownerAuthorized=false;authReady=false;document.body.classList.remove('owner-ready');document.body.classList.add('locked');$('ownerLogout').hidden=true;$('ownerAccount').hidden=true;$('ownerGate').hidden=true;$('ownerStatus').textContent=message;}
async function initIdentity(){if(readOnlyMode){publicUI();authReady=true;return}hideBrandIntro();$('ownerGate').hidden=true;try{const session=await authRequest();if(session.owner)ownerReady({email:'Área administrativa'});else ownerLocked()}catch{ownerLocked('Entre para enviar ou alterar arquivos.')}}
...
if($('brandEnter')&&readOnlyMode)$('brandEnter').onclick=()=>{if(publicLanding)location.href='/admin';else hideBrandIntro()};
publicUI();hideBrandIntro();if($('adminEntry')&&adminPath)$('adminEntry').onclick=event=>{event.preventDefault();showOwnerGate()};
...
async function openSharedModel(shareId){
 ...
 try{
  ...
  trustedReadOnlyLoad=true;try{if(!await load(file))return;}finally{trustedReadOnlyLoad=false}restoreDocuments(meta.state?.pdfs);restoreState(meta.state);$('filename').textContent=`${fileName} · somente visualização`;$('brandIntroHint').hidden=true;hideBrandIntro();status('');
